...
Padlock.Close = function() {
    ...
    $.post('https://flex-storerobbery/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://flex-storerobbery/PadLockClose');
    if (data.error != null) {
        $.post('https://flex-storerobbery/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://flex-storerobbery/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://flex-storerobbery/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://flex-storerobbery/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 ) {
            if (CurrentType == "keypad") {
                ...
                $.post('https://flex-storerobbery/exit');
            }
        }
    };
}); //docready
